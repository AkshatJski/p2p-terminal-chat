...
public final class HostNode extends Node {
    ...
    public void start() throws IOException {
        ...
        acceptThread.start();
        System.out.println("[System] Hosting on port " + port + ". Waiting for peers...");
    }
    ...
    private void acceptLoop() {
        while (running) {
            try {
                ...
                System.out.println("[System] Incoming connection from " + socket.getRemoteSocketAddress());
    ...
    private void serve(Socket socket) {
        ...
        try {
            ...
            wasPeer = true;
            System.out.println("[System] " + name + " connected from " + channel.remote());
    ...
    private void linkHost(String spec) {
        ...
        try {
            ...
            if (!trustGate.verifyClient(channel, endpoint, prompt)) {
                channel.close();
                System.out.println("[Bridge] Link to " + host + ":" + port + " not trusted.");
                return;
            }
            ...
            System.out.println("[Bridge] Linked to " + host + ":" + port
                    + " | fingerprint: " + shortFingerprint(channel.getFingerprint()));
            startLinkReader(link);
        } catch (Exception e) {
            System.out.println("[Bridge] Could not link to " + host + ":" + port + ": " + e.getMessage());
        }
    }
    ...
    private void acceptLink(SecureChannel channel, String first) {
        ...
        link.send(Protocol.command(Protocol.BRIDGE, roomSetCsv()));
        System.out.println("[Bridge] Host linked from " + channel.remote()
    ...
    private void linkLoop(HostLink link) {
        try {
            ...
            if (running) {
                System.out.println("[Bridge] Link " + link.endpoint + " lost: " + e.getMessage());
            }
        } finally {
    ...
    public void handleUserInput(String line) {
        ...
        switch (command) {
            ...
            case "@list" -> System.out.println("[Rooms] " + Protocol.roomList(roomListCsv()));
            case "@users" -> System.out.println("[Users in "
            ...
            default -> {
                if (self.room() == null) {
                    ...
                    System.out.println("[Room " + self.room() + "] " + username + ": " + cmd);
                }
            }
        }
    }
    ...
    public void close() {
        ...
        rooms.clear();
        byName.clear();
        System.out.println("[System] Host stopped.");
    }
